...
from ska_oso_services.common.calibrator_strategy import OBSERVATORY_CALIBRATION_STRATEGIES
...
TEST_TARGET = Target(
    ...
    name="SMC X-1",
    reference_coordinate=ICRSCoordinates(ra_str="01:17:05.14572884", dec_str="-73:26:36.01480816"),
)
...
def test_find_closest_calibrator_works_as_expected(mock_to_pdm_targets, dummy_calibrator_table):
    ...
    assert appropriate_calibrators[0].calibrator != appropriate_calibrators[1].calibrator
    assert appropriate_calibrators[0].calibrator.name == "3C 444"
    assert appropriate_calibrators[1].calibrator.name == "Pictor A"


@mock.patch("ska_oso_services.common.calibrators.to_pdm_targets")
def test_find_highest_calibrator_works_as_expected(mock_to_pdm_targets, dummy_calibrator_table):
